function Fix-DebugPrograms {
    ...
    Write-Verbose "Starting fix operation for Debug Programs right on $ComputerName"

    try {
        # First check current state
        $currentState = Check-DebugPrograms -ComputerName $ComputerName
        ...
        if ($result) {
            Write-Host "Successfully configured Debug Programs right for Administrators only" -ForegroundColor Green
            
            # Verify the fix
            $verifyResult = Check-DebugPrograms -ComputerName $ComputerName
